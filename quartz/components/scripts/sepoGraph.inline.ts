// @ts-nocheck
import { removeAllChildren, getFullSlugFromUrl, simplifySlug } from "@quartz-community/utils"
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      try {
        ...
        for (var key in dataRaw) {
          var normalizedKey = simplifySlug(key)
          if (!showTags && normalizedKey.startsWith("tags/")) continue
          data.set(normalizedKey, dataRaw[key])
        }
      } catch (err) {
      ...
      var darkgray = resolveColor(styles.getPropertyValue("--darkgray").trim(), "#4a4a4a")
      ...
      function graphNodeKind(d) {
        var tags = d.tags || []
        if (d.id.startsWith("tags/")) return "tag"
        if (d.id.startsWith("papers/") || tags.indexOf("paper") !== -1) return "paper"
        if (d.id.startsWith("notes/") || tags.indexOf("note") !== -1) return "note"
        return "page"
      }
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        var nodeKind = graphNodeKind(node)
        var isPaperNode = nodeKind === "paper"
        ...
        gfx.fill({ color: isTagNode || isPaperNode ? light : color })
        if (isTagNode) {
          gfx.stroke({ width: 2, color: tertiary })
        } else if (isPaperNode) {
          gfx.stroke({ width: 2, color: nodeStrokeColor(node), alpha: 0.9 })
        }
