...
exports.normalizeImplementIssueMetadata = normalizeImplementIssueMetadata;
...
const EXPLICIT_ROUTE_COMMANDS = ["answer", "implement", "fix-pr", "review", "orchestrate", "create-action"];
...
function normalizeOptionalBasePr(value) {
...
function fallbackImplementIssueBody(originalRequest) {
    return [
        "## Goal",
...
function normalizeImplementIssueMetadata(raw) {
    const text = (raw ?? "").trim();
    if (!text) {
 ...
 */
function buildRequestedRouteDecision(route, requestText, implementMetadata) {
    ...
    if (normalized.route === "orchestrate") {
        if (targetKind !== "issue" && targetKind !== "pull_request") {
            return {
                ...
                route: "unsupported",
                needsApproval: false,
                summary: "Orchestration requests are currently supported on issues and pull requests only.",
                issueTitle: "",
                issueBody: "",
            };
        }
