...
exports.SELF_MERGE_STATUS_MARKER = void 0;
exports.summarizeStatusChecks = summarizeStatusChecks;
exports.evaluateSelfMergeApproval = evaluateSelfMergeApproval;
exports.resolveSelfMerge = resolveSelfMerge;
exports.formatSelfMergeBody = formatSelfMergeBody;
exports.SELF_MERGE_STATUS_MARKER = "<!-- sepo-agent-self-merge -->";
...
function summarizeStatusChecks(checks) {
...
function evaluateSelfMergeApproval(input) {
...
function isCurrentlyMergeable(input) {
...
function canEnableAutoMerge(input) {
    const mergeState = normalizeToken(input.mergeStateStatus);
    const mergeable = normalizeToken(input.mergeable);
    if (mergeable === "conflicting" || mergeable === "false")
        return false;
    if (mergeState === "dirty" || mergeState === "draft" || mergeState === "behind")
        return false;
    return ["blocked", "clean", "has_hooks", "unknown", "unstable"].includes(mergeState);
}
function resolveSelfMerge(input) {
    let markReady = false;
...
function formatSelfMergeBody(input) {
    ...
    lines.push("", exports.SELF_MERGE_STATUS_MARKER);
    return lines.join("\n");
}
//# sourceMappingURL=self-merge.js.map