...
function sourceReviewNeedsFixPr(sourceAction, sourceConclusion, recommendedNextStep) {
    if (sourceAction.trim().toLowerCase() !== "review")
        return false;
    if ((0, handoff_js_1.normalizeRecommendedNextStep)(recommendedNextStep) === "human_decision")
        return false;
    return new Set(["minor_issues", "needs_rework", "changes_requested"]).has((0, handoff_js_1.normalizeConclusion)(sourceConclusion));
}
function sourceReviewRecommendedNextStep(sourceAction, rawResponse) {
    if (sourceAction.trim().toLowerCase() !== "review")
        return "";
    return (0, handoff_js_1.extractReviewRecommendedNextStep)(rawResponse);
}
...
if (!isManualOrchestrateStart && !orchestrationEnabled && !(0, handoff_js_1.automationModeAllowsHandoff)(automationMode)) {
    console.log("Skipping orchestrator dispatch: automation mode is disabled");
    process.exit(0);
}
const effectiveAutomationMode = orchestrationEnabled && !(0, handoff_js_1.automationModeAllowsHandoff)(automationMode)
    ? "heuristics"
    : automationMode;
const repo = process.env.GITHUB_REPOSITORY || "";
const ref = process.env.DEFAULT_BRANCH || "";
const rawResponse = readResponseFile();
const sourceConclusion = process.env.SOURCE_CONCLUSION || (0, handoff_js_1.extractReviewConclusion)(rawResponse) || "unknown";
const sourceRecommendedNextStep = (0, handoff_js_1.normalizeRecommendedNextStep)(process.env.SOURCE_RECOMMENDED_NEXT_STEP || sourceReviewRecommendedNextStep(sourceAction, rawResponse));
const sourceHandoffContext = process.env.SOURCE_HANDOFF_CONTEXT ||
    (sourceReviewNeedsFixPr(sourceAction, sourceConclusion, sourceRecommendedNextStep)
        ? (0, handoff_js_1.buildReviewFixPrHandoffContext)(rawResponse)
        : "");
