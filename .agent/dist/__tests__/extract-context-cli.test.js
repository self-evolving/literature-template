...
function runExtractContextCli(options) {
    ...
    try {
        ...
        (0, node_fs_1.writeFileSync)(eventPath, JSON.stringify(options.payload), "utf8");
        (0, node_fs_1.writeFileSync)(outputPath, "", "utf8");
        if (options.ghScript) {
            (0, node_fs_1.writeFileSync)((0, node_path_1.join)(tempDir, "gh"), options.ghScript, {
                encoding: "utf8",
                mode: 0o755,
            });
        }
...
const weakMentionCollaboratorCases = [
    {
        name: "issue comment",
...
for (const testCase of weakMentionCollaboratorCases) {
    (0, node_test_1.test)(`extract-context promotes weak ${testCase.name} associations for repository collaborators`, () => {
        ...
        node_assert_1.strict.equal(outputs.get("should_respond"), "true");
        node_assert_1.strict.equal(outputs.get("association"), "COLLABORATOR");
        node_assert_1.strict.equal(outputs.get("source_kind"), testCase.expectedSourceKind);
...
(0, node_test_1.test)("extract-context preserves weak discussion comment association when collaborator lookup fails", () => {
    const outputs = runExtractContextCli({
        eventName: "discussion_comment",
        payload: {
            sender: { login: "alice", type: "User" },
